...
import ArrowCard from "./ArrowCard.tsx";
import { cn } from "../lib/utils";
import type { Post } from "../lib/hashnode/schema.ts";
...
export default function Blog({ data, tags }: Props) {
  ...
  const [posts, setPosts] = createSignal<{ node: Post }[]>([]);

  createEffect(() => {
    setPosts(
      data.filter((entry) =>
        Array.from(filter()).every((value) =>
          entry.node.tags.some(
            (tag) => tag.name.toLowerCase() === String(value).toLowerCase()
          )
        )
      )
    );
      ...
      <div class="col-span-3 sm:col-span-2">
        <div class="flex flex-col">
          ...
          <ul class="flex flex-col gap-3">
            {posts().map((post) => (
              <li>
                <ArrowCard entry={post.node} collection="blog" />
              </li>
            ))}
          </ul>
        </div>
